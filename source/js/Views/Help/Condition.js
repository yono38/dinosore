window.$dino.HelpConditionView = Backbone.View.extend({
	...
	createBugIntro: function(e) {
        ...
        $dino.intro = intro = introJs();
          ...
          intro.oncomplete(function(){
          	$dino.app.navigate("help?type=condition&page=2", {
          		trigger: true
          	});
	...
	createNewConditionIntro: function(){
        ...
        $dino.intro = intro = introJs();
          ...
          var nextpage = _.once(function(){
          	...
          	$dino.app.navigate("help?type=condition&page=3", {
          		trigger: true
          	});
          });
          intro.onbeforechange(function(target){
	...
	createConditionListItemIntro: function() {
		...
		var item = new $dino.Bug({
			...
			details: "Stuff",
			title : "Migraines",
			symptom : [{title: 'Itchy Scalp', id: '-1'}, {title: 'Headache', id: '-2'}, {title: 'Blurred Vision', id: '-3'}],
			doctor: {title: "Dr. Roberts"},
			medication : []
		});
		var itemview = new $dino.ConditionListItemView({
			model : item
		});
        ...
        $dino.intro = intro = introJs();
			...
			var nextpage = _.once(function(){
	          	...
	          	$(".introjs-overlay").remove();
	          	$(".introjs-helperLayer").remove();
	          	$dino.app.navigate("help?type=appointment&page=1", {
	          		trigger: true
	          	});
          	});          
	...
	renderNewConditionView: function() {
		var view = this.views[1] = new $dino.ConditionNewView({
			header : "New"
		});

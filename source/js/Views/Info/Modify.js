window.$dino.InfoModifyView = $dino.NewFormView.extend({
	...
	// remove item from DB, then from HTML DOM
	...
	loadUserList: function(type, selector) {
		...
		this[type+"List"].fetch({
			...
			success : function(collection) {
				if (type == "doctor") console.log(collection);
	...
	// overrides parent to make li list instead of select menu
	makeList: function(collection, selector, type){
		collection.each(function(item, idx, coll){
			this.$(selector).append('<li><a href="#" data-role="button" data-inline="true" data-icon="delete" data-id="'+item.id+'" class="delete_detail_item" data-type="'+type+'" data-iconpos="notext" data-theme="a" title="X">X></a>' 
			 + '<span class="fancyFont itemName"><span data-id="'+item.id+'" class="itemName">'+item.get("title")+'</span></span></li>');
